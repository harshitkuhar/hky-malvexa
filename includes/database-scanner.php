<?php
/**
 * WP Doctor Procedural Database Forensics Scanner
 ...
 */
function wpdoctor_get_target_db( $site_id = 0 ) {
	global $wpdb;
 ...
 */
function wpdoctor_scan_database( $site_id = 0 ) {
	$db = wpdoctor_get_target_db( $site_id );
	$findings = array();
	$rules = wpdoctor_get_malware_rules();
	...
	if ( $has_snippets_table === $snippets_table ) {
		...
		if ( ! empty( $snippets ) ) {
			foreach ( $snippets as $sp ) {
				...
				$is_neutralized = ( stripos( $code, 'WP DOCTOR NEUTRALIZED MALWARE' ) !== false );
	...
	if ( ! empty( $options_query ) ) {
		foreach ( $options_query as $opt ) {
			...
			if ( empty( $val ) || stripos( $val, 'WP DOCTOR NEUTRALIZED MALWARE' ) !== false ) {
				continue;
			}
	...
	if ( ! empty( $posts_query ) ) {
		foreach ( $posts_query as $post ) {
			...
			$is_neutralized = ( stripos( $content, 'WP DOCTOR NEUTRALIZED MALWARE' ) !== false );
